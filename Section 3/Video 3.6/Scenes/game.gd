...
func remove_piece_at_tile(tile: BoardTile) -> void:
	...
	# check goes here! :D
	if get_tree().get_nodes_in_group("white_pieces").size() == 0:
		win_game("black")
	if get_tree().get_nodes_in_group("black_pieces").size() == 0:
		win_game("white")
		
func win_game(team: String) -> void:
	data.set_winning_team(team)
